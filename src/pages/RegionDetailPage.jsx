...
import { contactInfo } from "../data/sharedSiteData";
import { createBreadcrumbSchema, createFaqSchema, createServiceSchema } from "../lib/seoSchemas";

function getAreaServed(listPath, page) {
  return listPath === "/sehirler" ? page.district : `${page.district}, Kayseri`;
}

function getParentLabel(listPath) {
  return listPath === "/sehirler" ? "Şehirlerarası" : "Hizmet Bölgeleri";
}
      ...
      <Seo
        ...
        jsonLd={[
          createBreadcrumbSchema([
            { name: "Ana Sayfa", path: "/" },
            { name: getParentLabel(listPath), path: listPath },
            { name: page.title, path: `${listPath}/${page.slug}` }
          ]),
          createServiceSchema({
            name: page.title,
            ...
            areaServed: getAreaServed(listPath, page)
          }),
