        ...
        <div className="service-grid">
          {services.map((service) => (
            <article key={service.title} className="service-card reveal">
              <h3>
                <Link to={`/hizmetler/${service.slug}`}>{service.title}</Link>
              </h3>
      ...
      <div className="section-shell split-layout">
        <div className="reveal">
          ...
          <p className="section-text">
            Nakliyatta güven; doğru ekipman, deneyimli personel ve açık iletişimle oluşur. AZR Evden Eve Nakliyat olarak bu üç alanı birlikte yönetiyoruz.
          </p>
        </div>
...
export function LocalSeoSection({ blocks, badges }) {
  return (
    <section className="section-block">
      <div className="section-shell local-seo-layout">
        ...
        <div className="detail-layout local-seo-cards">
          {blocks.map((item) => (
        ...
        <div className="preview-grid">
          {groups.map((group) => (
            <article key={group.title} className="preview-card reveal">
              ...
              <div className="chip-grid">
                {group.items.map((item) => (
                  <Link
                    ...
                    to={item.to || "#"}
                    state={{ selectedPlanningTopic: item.label }}
                    className="chip-item chip-link"
                  >
                    {item.label}
                  </Link>
                ))}
              ...
              {group.action ? (
                <div className="service-area-action">
                  <Link to={group.action.to} className="btn btn-secondary alt-dark">
                    {group.action.label}
                  </Link>
                </div>
              ) : null}
